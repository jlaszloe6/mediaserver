...
Ebooks:     drop .torrent in /watch ──► ebook-pipeline (RPC add) ──► Transmission ──► ebook-pipeline (convert+organize) ──► /media/ebooks ──► Audiobookshelf ──► app / Kindle
```
...
1. `mkdir -p "$MEDIA_ROOT"/media/{music,audiobooks,ebooks} "$MEDIA_ROOT"/watch "$MEDIA_ROOT"/torrents/complete/ebooks-incoming`
