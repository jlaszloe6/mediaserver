...
Ebooks have no acquisition pipeline either — add files to `/media/ebooks` directly (e.g. from a Calibre export) and trigger an Audiobookshelf library scan. **epub is the standard format for this library**: Audiobookshelf only tracks reading progress for epub, not mobi/azw3, and epub is the safest bet for Amazon's Send-to-Kindle (which doesn't officially support azw3 or the older prc/Mobipocket extension). Convert non-epub imports with Calibre's `ebook-convert` before adding them — a throwaway `lscr.io/linuxserver/calibre` container works well for one-off batch conversions, no need to install Calibre on the host. Keep one book per `Author/Title/` folder (never mix a flat ebook file directly under an author folder alongside title subfolders) — Audiobookshelf's folder-as-item detection gets confused by the mix and misfiles the item.
