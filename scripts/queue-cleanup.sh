...
send_email() {
    ...
    # so a transient SMTP failure previously killed queue-cleanup.sh right
    # here - before the if/else below (and the script's own final ERRORS
    # exit-code check) ever ran.
    local send_rc=0
    curl -sf --max-time 30 --url "smtp://$SMTP_SERVER:$SMTP_PORT" \
...
# Send consolidated email if there were any issues. `if ! send_email ...;
# then` (not a bare statement) - send_email now returns 1 on a failed
# send, and calling it as an if-condition means that failure can't abort
# the script here under set -e. EMAIL_ERRORS is tracked separately from
# ERRORS (a failed notification about the queue state is a different
# problem than a failed queue fix - the final summary below names each
# distinctly) but still counts toward a non-zero exit: a silently-broken
# notification channel is itself worth surfacing to cron/monitoring, not
# just to whoever happens to read this log.
EMAIL_ERRORS=0
if [ -n "$ALERT_MESSAGES" ]; then
    log "Sending alert email..."
    if ! send_email "[${SERVER_NAME:-Media Server}] Queue issues detected" "The following issues were found in the download queue:

$ALERT_MESSAGES
---
Generated by queue-cleanup.sh at $(date '+%Y-%m-%d %H:%M:%S')"; then
        log "WARN: Alert email failed to send (queue fixes above, if any, still applied)"
...
EXIT_CODE=0
if [ "$ERRORS" -gt 0 ]; then
    DONE_MSG="${DONE_MSG} with $ERRORS cleanup error(s)"
    EXIT_CODE=1
fi
if [ "$EMAIL_ERRORS" -gt 0 ]; then
    if [ "$ERRORS" -gt 0 ]; then
        DONE_MSG="${DONE_MSG} and $EMAIL_ERRORS email failure(s)"
    else
        DONE_MSG="${DONE_MSG} with $EMAIL_ERRORS email failure(s) (cleanup otherwise OK)"
    fi
    EXIT_CODE=1
fi
log "${DONE_MSG} ==="
