...
| `*/5 * * * *` | `jellyfin-scan.sh` | Trigger Jellyfin library scan (covers manual additions) |
...
| `*/5 * * * *` | `subtitle-preextract.sh` | Force embedded-subtitle extraction right after import, before anyone plays it |
...
- Root fix (not just a mitigation): `scripts/subtitle-preextract.sh` (new, every 5 min, mirrors `subtitle-sync-check.sh`'s Sonarr/Radarr import-watching pattern) forces embedded-subtitle extraction right after a video is imported — hitting Jellyfin's own subtitle-stream endpoint (`GET /Videos/{itemId}/{mediaSourceId}/Subtitles/{index}/Stream.srt`, the same code path playback uses) once nothing else is competing for that file's NFS bandwidth, instead of leaving extraction to happen on demand at playback time. By the time anyone presses play, the `.srt` is already cached on local disk (`/config/data/data/subtitles/...` — note this output path was *never* on NFS; only the source video read was) and playback never needs to extract it live at all. Resolves Sonarr episodes and Radarr movies by TVDB/TMDB id, matching `jellyfin-watched-cleanup.sh`'s established lookup pattern; an item not yet visible in Jellyfin (scan hasn't caught up) retries the whole batch next run rather than erroring
