...
- Fix: NAS traffic (`192.168.1.76`) is now pinned to the wired NIC via a host route (`/etc/NetworkManager/dispatcher.d/99-nas-via-wired.sh`, a NetworkManager dispatcher script so it reapplies on every reboot/reconnect) — installed live via SSH 2026-09-01, then codified in `server-setup.sh` (`WIRED_IFACE` env var, optional — skipped on hosts with no spare wired NIC) for future fresh provisions. WiFi keeps the default route for everything else, so client-facing streaming is unaffected
