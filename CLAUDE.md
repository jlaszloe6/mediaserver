...
- `nas-route-setup.sh` also sets `never-default`/a high route-metric for **both** `ipv4` and `ipv6` on the wired connection profile — they're separate NetworkManager properties (`ipv6.never-default` defaults to off, i.e. IPv6 stays a default-route candidate, unlike `ipv4.never-default`), so setting only the IPv4 one would still let an IPv6 RA/gateway on that NIC hijack the default route on its own. Without either, general LAN traffic could drift onto the wired NIC despite the NAS-only design
